...
//GetOutboundIP Get prefered outbound ip of this machine
...
func route(w http.ResponseWriter, r *http.Request) {
	...
	if resp.StatusCode == http.StatusOK {
		...
		redirectRequest.Header.Set("Content-Type", "applocation/json")
		rediretResponse, err := http.DefaultClient.Do(redirectRequest)
		if err != nil {
			panic(err)
		}
		defer rediretResponse.Body.Close()
		if rediretResponse.StatusCode == http.StatusOK {
			bodyBytes, _ := ioutil.ReadAll(rediretResponse.Body)
			bodyString := string(bodyBytes)
			fmt.Println(bodyString)
